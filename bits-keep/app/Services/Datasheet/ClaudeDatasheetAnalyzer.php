...
/**
 * Claude CLI をサーバ上で実行してデータシートを解析する。
 * PDF は CLI へ直接渡せないため、テキストまたはページ画像へ変換し、
 * 作業ディレクトリ内のファイルとして読ませる。
 */
class ClaudeDatasheetAnalyzer implements DatasheetAnalyzer
{
    /**
     * 目的: 解析に必要な依存オブジェクトを受け取る。
     * 機能: プロンプト正本、PDF変換、Claude実行、結果正規化を保持する。
     * 入力: 各依存オブジェクト。
     * 出力: インスタンス。
     * 動作条件: なし。
     * 副作用: なし。
    ...
    /**
     * 目的: Claude 実行環境が整っているかを返す。
     * 機能: Claude CLI の前提確認結果をそのまま返す。
     * 入力: なし。
    ...
    /**
     * 目的: データシートPDFを Claude CLI で解析し、正規化済みの結果を返す。
     * 機能: 専用作業ディレクトリを作り、PDFを変換して Claude を実行し、結果を正規化してから作業ディレクトリを削除する。
    ...
    {
        $root = rtrim((string) config('datasheet.claude.workspace_root'), '/');
        if ($root === '') {
            throw DatasheetAnalysisException::environment('解析用の作業ディレクトリが設定されていません。');
        }

        // リポジトリ配下を作業根にすると CLAUDE.md などが読み込まれて解析指示が汚染される
