...
class NetworkSearchService
{
        ...
        return Component::whereHas('categories', fn ($q) => $q->where('name', 'like', "%{$categoryKeyword}%"))
            ...
            ->flatMap(function ($component) {
                $spec = $component->specs
                    ->whereIn('value_profile', ['typ', 'triple'])
                    ->firstWhere('value_numeric_typ', '!=', null);
                if (! $spec?->value_numeric_typ) {
                    return [];
                }

                return [[
                    'value' => (float) $spec->value_numeric_typ,
    ...
    private function formatValue(float $value, string $partType): string
    {
        if ($partType === 'C') {
            if ($value < 1e-9) {
                return round($value * 1e12, 3).'pF';
            }
            if ($value < 1e-6) {
                return round($value * 1e9, 3).'nF';
            }
            if ($value < 1e-3) {
                return round($value * 1e6, 3).'μF';
            }

            return round($value * 1e3, 3).'mF';
        }
        if ($value < 1e3) {
            return round($value, 3).'Ω';
        }
        if ($value < 1e6) {
            return round($value / 1e3, 3).'kΩ';
        }
