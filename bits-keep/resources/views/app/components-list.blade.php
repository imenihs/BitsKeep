...
<div id="app" data-page="components-list" class="flex w-full min-h-screen">
  ...
  <!-- ===== 左サイドバー: フィルター ===== -->
  <aside class="w-64 flex-shrink-0 border-r border-[var(--color-border)] p-4 overflow-y-auto bg-[var(--color-card-even)]">
    ...
    <div class="border-t border-[var(--color-border)] pt-4">
      <button @click="advancedOpen = !advancedOpen"
        class="flex items-center justify-between w-full text-xs font-semibold opacity-60 mb-2">
  ...
  <main class="flex-1 flex flex-col overflow-hidden">
    ...
    <div class="p-4 border-b border-[var(--color-border)] flex items-center gap-3 flex-shrink-0">
      ...
      <a v-if="alertCount > 0" href="/stock-alert"
        class="flex items-center gap-1.5 px-3 py-1.5 rounded text-sm border border-[var(--color-tag-warning)] text-[var(--color-tag-warning)]">
        ⚠ {{ alertCount }}件
      </a>
      <!-- 比較バー -->
      <div v-if="compareList.length" class="flex items-center gap-2 px-3 py-1.5 rounded bg-[var(--color-card-even)] border border-[var(--color-border)] text-sm">
        比較: {{ compareList.length }}件
        <a href="/component-compare" class="btn btn-primary text-xs px-2 py-1 rounded">比較する</a>
      </div>
