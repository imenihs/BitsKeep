    ...
    <div class="flex-1 min-w-0 space-y-12 text-sm leading-relaxed">
      ...
      <section id="master">
        ...
        <ul class="list-disc list-inside space-y-1 mb-5 opacity-80">
          ...
          <li>スペック詳細を新規追加するとき、基準単位欄には <code>uF</code> や <code>kΩ</code> ではなく <code>F</code> や <code>Ω</code> を入力します。値欄で使える <code>u</code> / <code>k</code> などは入力候補接頭辞で選びます</li>
          ...
          <li>登録画面のスペック入力では、スペック詳細の基準単位を単位欄に使い、接頭語は値欄へ付けます。<code>1u</code> + <code>A</code> は <code>0.000001 A</code>、<code>1k</code> + <code>Ω</code> は <code>1000 Ω</code> として扱います</li>
          <li>部品登録画面のスペック行に自由な「名前」欄はありません。必ずスペック詳細を選び、候補にない場合だけスペック追加カード下段の「スペックを新規で追加」からスペック詳細を登録します</li>
      ...
      <section id="tools">
        ...
        <ul class="list-disc list-inside space-y-1 mb-5 opacity-80">
          ...
          <li>通常分圧とVR分圧は分圧メインタブ内で切り替えます。設計解析ツールのNTC/PTC温度変換は温度-電圧グラフ、感度の良い/悪い温度域、測定対象温度範囲の感度、自己発熱、抵抗公差/サーミスタ公差の破線範囲、ADCコード表、線形化、固定抵抗候補に責務を絞り、通常分圧/VR分圧候補は分圧タブのエンジンを使います</li>
          <li>許容誤差（<code>tolerance_pct</code>）は目標値への探索許容範囲です。部品許容差（<code>element_tolerance_pct</code>）は、採用した各公称部品のばらつき確認用の条件です</li>
          <li>候補には公称合成値、独立ばらつきのRSS目安範囲、保証確認用の全素子同方向コーナー範囲を表示します。RSS目安は各素子の正規化感度から <code>sqrt(sum((S_i * 許容差)^2))</code> で求め、コーナー範囲は全素子を下限値/上限値として同じトポロジを評価します</li>
          ...
          <li>分圧は R1/R2、VR調整ありは R上/VR/R下 の許容差を個別に設定でき、負荷込み出力の独立RSS目安範囲とコーナー範囲を表示します</li>
          ...
          <li>可変抵抗 + 固定抵抗は、固定抵抗とVRそれぞれの許容差を設定でき、要求下限/上限端の独立RSS目安範囲とコーナー範囲を表示します</li>
          ...
          <li>現実装は公称値と無負荷/指定負荷モデルです。RSS目安は独立ばらつきの設計目安で保証値ではなく、全素子同方向コーナーは保証境界確認用で起きやすさを示すものではありません。温度係数、電圧係数、経年変化、寄生成分、同一ロット相関、実測分布、モンテカルロ統計、VRワイパ抵抗、電力、負荷の動的変動などは別途確認が必要です。設計確認が残る候補はPASSにせず、範囲を含む候補はCHECK、足りない候補はWARNで表示します</li>
        </ul>
        ...
        <p class="mb-2 opacity-80">ネットワーク探索・分圧・可変抵抗・EIA-96早見表・ADC 設計・電解コンデンサ寿命推定・NTC/PTC温度変換・電流検出・インタフェース余裕解析・電源余裕解析・バッテリー稼働時間・ロジックIC参照・コネクタ設計/ピン配置・0Ω/未実装/ジャンパ整理など、専門的な設計解析ツールがタブで並んでいます。</p>
        <ul class="list-disc list-inside space-y-1 opacity-80">
          <li>ツール入口は <span class="help-code">すべて</span> <span class="help-code">受動部品</span> <span class="help-code">計測・変換</span> <span class="help-code">余裕・信頼性</span> <span class="help-code">保護・起動</span> <span class="help-code">参照・実装</span> の設計目的で絞り込めます</li>
          <li>各ツールは <span class="help-code">仕様・前提確認</span> → <span class="help-code">パラメータ入力</span> → <span class="help-code">結果確認</span> → <span class="help-code">次アクション</span> の順に確認できます。通常入力は各ツール内で行い、共通詳細入力に残っていた最悪条件、定格、保存用条件の補足はツール別の <span class="help-code">追加条件</span> へ統合します。同じ条件を共通欄と通常欄へ二重に出さず、1条件1入力欄で扱います</li>
          <li>画面上も、回路前提と確認記号、各ツールの入力・結果プレビュー、ツール別追加条件、最終判定と保存導線の順に並びます。個別結果は即時計算のプレビュー、最終判定カードはverdict、支配要因、不足条件、次アクション、コピー、保存の出口です。コネクタ設計では、入力フォームの後にピンマップとユーザー登録を確認します</li>
          <li>数値入力欄は <code>10k</code>、<code>100n</code>、<code>10m</code>、<code>1T</code>、<code>512KiB</code> のような工学接頭語付き入力を受け付けます。単位はラベル側をV/A/Ω/F/s/Wなどの基準単位へ寄せ、初期値・サンプル値は単位文字を付けずに数値と接頭語だけで表示します。ユーザー入力では <code>10kΩ</code>、<code>100nF</code>、<code>10mA</code> も受け付けます。<code>meg</code> / <code>MEG</code> は <code>M</code> と同じ倍率として扱います</li>
          <li>ツールタブは受動部品目的のネットワーク探索から始まります。前回開いたツールがある場合は、そのツールの目的に合わせて復元します。目的を切り替えたときに現在のツールが含まれなければ、その目的内の先頭ツールへ移動します。目的フィルタ、並び順、最後に開いたツールはブラウザに保存され、初期状態へ戻せます</li>
          <li>抵抗/容量ネットワーク探索、分圧、可変抵抗は設計解析ツール群の一部です。設計解析ツール内ではネットワーク探索、分圧、可変抵抗をメインタブとして扱い、分圧内だけVR調整なし/VR調整ありを切り替えます。NTC/PTC温度変換は温度モデル、温度-電圧グラフ、感度確認、自己発熱、公差振れ、ADC変換へ責務を分けます</li>
          <li>EIA-96早見表は、チップ抵抗器の3文字マーキングを2桁インデックスと倍率文字から抵抗値へ変換します。Z/Y/X/A/B/C/D/E/Fに加え、R/S/Hの別表記も受け付け、倍率別の96行表、抵抗値から近いコード、全倍率検索を表示します</li>
          <li>電流検出はローサイド・シャント、ケルビン配線、電流検出アンプ、ADCの位置関係を確認し、片方向オフセットなし/双方向0Aオフセットありを切り替えます。入力電流からVshunt、Vout、ADCコードをシミュレーションし、最小/最大電流時のアンプ出力、ADC/アンプ余裕、Rs定格余裕を見ます。Rs定格超過はFAIL理由として明示します</li>
          <li>電源余裕は通常負荷合計と供給電力の余裕を直接表示し、効率は子レールから親レールへの上流換算に使います。負荷ごとにレール名を指定でき、未指定時だけ電圧一致で割り当てます。レールツリー、dropout、最大負荷シナリオ、支配負荷を確認し、突入電流は時間幅と対象レールが未確定ならCHECK条件として残します</li>
          <li>バッテリー稼働時間は、電池パック、動作条件、周期負荷、結果の順に確認します。電池パックは種類・セル数・容量(Ah)・公称電圧・内部抵抗・使用可能容量、動作条件は周期時間・システム最低電圧・要求稼働時間、周期負荷は名称・負荷電圧(V)・負荷電流(A)・変換効率(%)・ON時間(s)を扱います。負荷ごとの入力セットは横並びで、セットを縦に追加できます。ON秒数合計は同時ON負荷を表すため周期時間超過だけではFAILにしません。結果は実効稼働時間と制限要因を主表示し、Wh/周期、平均電力、平均電流、全負荷同時ON前提のピーク負荷、容量ベース時間、電圧下限到達時間を根拠値として表示します。電圧降下グラフの右側には、電池100%を使い切るまでの容量消費内訳円グラフを置き、各負荷のWh/周期比率が総消費の何%を占めるかを表示します。システム最低電圧が標準下限より低い場合は終端曲線を外挿して反映し、深放電目安としてWARNします。電圧降下グラフは内部抵抗とピーク負荷を反映し、右端を負荷時バッテリー電圧がシステム最低電圧へ到達する交点に固定します。グラフの数値表示はSVG内tooltipへ一本化し、24h超は日、30日超は月、365日超は年スケールも追加表示します</li>
          <li>比較器はVin入力先を+入力(非反転)/-入力(反転)で切り替え、Vref方式とVcc分圧方式を組み合わせます。-入力+VCC分圧だけはR1をショートし、R2||R4のテブナン抵抗とR3でヒステリシスを計算します。他方式は従来通りR1を使います</li>
          <li>熱設計はP発熱源からRthチェーンを通ってTa周囲温度へ流れる向きで表示します。通常Tj超過のみFAIL、ワースト倍率やディレーティング不足はWARNとして扱います</li>
          <li>有識者レビューでは、機能の有無だけでなく、仕様・前提確認、入力導線、結果確認、次アクション、画面上の仕様感も確認します</li>
          <li>結果カードには PASS/WARN/FAIL/CHECK、支配要因、不足条件、次アクション、コピー用サマリが表示されます。WARN/FAIL/CHECKのsummaryは、違反したパラメータや不足条件を先頭に出します</li>
          <li>実装は設計ツール本体、機能別JSモジュール、Blade partialに分割しています。部品登録/詳細、マスタ管理、エンジニア電卓、抵抗/容量探索、NetworkSearch、UiApiSurfaceSmoke、Tampermonkey補助も責務単位で分割し、一次コードは1000行目安を超えない構成にしています。公開関数、主要ヘルパー、副作用を持つ操作関数には目的、入力、出力、動作条件、副作用をコメントで残します</li>
          <li>条件不足のTVS、ヒューズ、保護協調、ロジックIC参照、コネクタ定格未確認は PASS にせず CHECK と不足条件を返します</li>
          <li>ロジックIC参照は 74LS/74F/74HC/74HCU/74HCT/74VHC/74VHCT/74LC/74LVC/4000/4500/5000 系を対象に、ゲート、インバータ/シュミット、バッファ、FF/ラッチ、カウンタ、シフトレジスタ、デコーダ/エンコーダ、MUX/DEMUX、アナログスイッチなど広い機能カタログから候補を返します。シリーズ接続の電圧レベル判定はIF余裕で扱います</li>
          <li>IF余裕では、送信側シリーズ+Vcc と受信側シリーズ+Vcc から VOH/VOL/VIH/VIL の近似値と受信側の入力耐圧/5V tolerant 条件を使って、レベル余裕と入力過電圧リスクを判定します。静的近似のため、入力耐圧、出力電流、伝搬遅延、速度、温度範囲、ピン配置、負荷条件、未使用入力の固定方法は最終的に候補型番のデータシート条件で確認します</li>
          <li>コネクタ参照はコネクタ設計/ピン配置として実装済みです。USB、D-sub、IDC、ピンヘッダ、JST、Molex、RJ45などの標準テンプレートに加え、ユーザー登録の写真URL、図URL、データシートURL、ピン番号規則、定格、相手側部品を扱います</li>
          <li>コネクタ設計/ピン配置では、写真/図つきピンマップ、ピン割付、信号名、電圧、電流、並列電源ピン、シールド、差動ペア、ケーブルAWG、derating、BOM/シルク注記を確認し、定格根拠や視点図が不足する場合はPASSにしません</li>
          <li>登録部品を指定すると、その部品のスペック値を解析入力へ反映できます</li>
          <li>解析条件は案件・部品・BOM行と一緒に保存できます。案件は案件名/事業名/案件番号検索、部品は型番/通称/メーカー検索で選択し、ID直接入力は使いません</li>
          <li>保存済み解析は一覧から title/verdict/summary/案件/部品/BOM/更新日を確認でき、差分表示、入力へ復元、削除確認まで同じ保存パネルで操作できます</li>
