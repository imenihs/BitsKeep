    ...
    <div class="flex-1 min-w-0 space-y-12 text-sm leading-relaxed">
      ...
      <section id="tools">
        ...
        <ul class="list-disc list-inside space-y-1 mb-5 opacity-80">
          ...
          <li>分圧タブは分圧とVR分圧を切り替えます。分圧は2抵抗分圧、VR分圧は Vin → R上 → VR → R下 → GND のワイパ出力範囲設計です</li>
          <li>分圧は比率指定に加え、入力電圧 Vin と出力電圧 Vout から目標比率を直接計算できます</li>
          <li>通常分圧は負荷条件を指定できます。抵抗負荷は出力からGNDへの抵抗で∞は無負荷、電流負荷は出力から引き抜く電流で0Aは無負荷です。電流負荷の計算には Vin が必要です</li>
          <li>無負荷は記号入力に依存せず、抵抗負荷は∞ボタン、電流負荷は0Aボタンで設定できます。この操作は分圧とVR分圧の両方で使えます</li>
          <li>分圧は R1/R2、実比率、負荷込み出力電圧、総抵抗、誤差を表示します</li>
          ...
          <li>VR分圧は、出力下限/上限と指定した基準VR値から R上/R下 の固定抵抗候補を出し、負荷込みで端点を計算します。基準VR値は設計で使うVR素子の公称値として固定し、10kΩ指定時に200Ωのような大きく異なるVR値は採用しません</li>
