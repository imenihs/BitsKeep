  ...
  <div class="flex gap-10">
    ...
    <div class="flex-1 min-w-0 space-y-12 text-sm leading-relaxed">
      ...
      <section id="tools">
        ...
        <p class="mb-2 opacity-80">抵抗/容量ネットワーク、分圧、可変抵抗の計算を同じ画面で扱います。設計解析ツールハブの受動部品ネットワーク/分圧タブにも同じ操作面が入り、別ページへ移動せずに候補を確認できます。</p>
        <ul class="list-disc list-inside space-y-1 mb-5 opacity-80">
          <li>上位タブはネットワーク探索、分圧、可変抵抗です</li>
          <li>ネットワーク探索タブは抵抗/容量ネットワーク専用です。分圧UIは置かず、抵抗と容量で直列/並列の合成式を切り替えます</li>
          <li>通常分圧とVR分圧は受動部品ネットワーク/分圧タブのサブモードです。設計解析ツールのNTC/PTC温度変換は温度スイープ、ADCコード表、線形化、プルアップ/プルダウン候補に責務を絞り、通常分圧/VR分圧候補は受動部品タブの分圧エンジンを使います</li>
        ...
        <p class="mb-2 opacity-80">受動部品ネットワーク/分圧設計・ADC 設計・電解コンデンサ寿命推定・NTC/PTC温度変換・電流検出・インタフェース余裕解析・電源余裕解析・ロジックIC参照・コネクタ設計/ピン配置・0Ω/未実装/ジャンパ整理など、専門的な設計解析ツールがタブで並んでいます。</p>
        <ul class="list-disc list-inside space-y-1 opacity-80">
          ...
          <li>数値入力欄は <code>10kΩ</code>、<code>100nF</code>、<code>3.3uV</code>、<code>1T</code>、<code>512KiB</code> のような工学接頭語付き入力を受け付けます。SI は <code>Y/Z/E/P/T/G/M/k/無印/m/u/n/p/f</code>、IEC は <code>Ti/Gi/Mi/Ki</code> を扱います</li>
          <li>抵抗/容量ネットワーク探索は設計解析ツール群の一部です。受動部品ネットワーク/分圧タブ内で、R/C探索、通常分圧、VR分圧、可変抵抗を独立したサブモードとして扱い、NTC/PTC温度変換は温度モデルとADC変換へ責務を分けます</li>
          ...
          <li>ロジックIC参照は 74LS/74F/74HC/74HCU/74HCT/74VHC/74VHCT/74LC/74LVC/4000/4500/5000 系を対象に、ゲート、インバータ/シュミット、バッファ、FF/ラッチ、カウンタ、シフトレジスタ、デコーダ/エンコーダ、MUX/DEMUX、アナログスイッチなど広い機能カタログから候補を返します</li>
          <li>送信側シリーズ+Vcc と受信側シリーズ+Vcc から VOH/VOL/VIH/VIL の近似値と受信側の入力耐圧/5V tolerant 条件を使って、レベル余裕と入力過電圧リスクを判定します。静的近似のため、入力耐圧、出力電流、伝搬遅延、速度、温度範囲、ピン配置、負荷条件、未使用入力の固定方法は最終的に候補型番のデータシート条件で確認します</li>
