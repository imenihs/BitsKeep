...
<div id="app" data-page="component-detail" data-id="{{ $id }}" class="p-6 max-w-4xl mx-auto">
  ...
  <!-- 出庫モーダル -->
  <div v-if="stockOutModal.open" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="stockOutModal.open = false">
  ...
  <!-- 入庫モーダル -->
  <div v-if="stockInModal.open" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="stockInModal.open = false">
    <div class="bg-[var(--color-bg)] rounded-xl shadow-xl p-6 w-full max-w-sm border border-[var(--color-border)]">
